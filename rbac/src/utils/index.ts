...
const subscribeMessage = async (channel: amqplib.Channel) => {
    ...
    channel.consume(appQueue.queue, async (data: any) => {
        if (data !== null) {
            ...
            console.log('received data from rbac1',data.content.toString())
            ...
            if (message.service === "film") {
                await publishMessage(channel, config.FILM_BINDING_KEY, JSON.stringify({service:"rbac",status:grant === true ? "success" : "denied"}))
            }
            channel.ack(data)
        }
    })
